...
# fork from somewhere around 2.1.0, with various C++ source files added.
...
# do the same for PrusaSlicer-gcodeviewer
convert -set filename:dim '%%wx%%h' ../resources/icons/PrusaSlicer-gcodeviewer.ico prusa-slicer-gcodeviewer-%%[filename:dim].png
for res in 16 32 48 64 128 256; do
    mkdir -p ${res}x${res}/apps
    cp prusa-slicer-gcodeviewer-${res}x${res}.png ${res}x${res}/apps/prusa-slicer-gcodeviewer.png
done
rm prusa-slicer-gcodeviewer-*.png
popd
...
# move the .desktop files to the right place
mv %buildroot%_datadir/PrusaSlicer/applications %buildroot%_datadir/applications
sed -i 's/Icon=PrusaSlicer/Icon=PrusaSlicer/' %buildroot%_datadir/applications/PrusaSlicer.desktop
sed -i 's/Icon=PrusaSlicer/Icon=PrusaSlicer/' %buildroot%_datadir/applications/PrusaGcodeviewer.desktop
...
%_datadir/icons/hicolor/*/apps/%name-gcodeviewer.png
%_datadir/applications/PrusaSlicer.desktop
%_datadir/applications/PrusaGcodeviewer.desktop
%_datadir/appdata/%name.appdata.xml
%dir %_datadir/PrusaSlicer
%_datadir/PrusaSlicer/{data,icons,models,profiles,shaders,udev}/


%changelog
* Thu Jan 21 2021 Alexander Jacocks <alexander@redhat.com> - 2.3.0-1
- Update to 2.3.0.
- Remove endian patch as it has been integrated in admesh
- Update included library versions
- Remove Fedora-specific .desktop file in favor of upstream
- Fixes: rhbz#1831786
